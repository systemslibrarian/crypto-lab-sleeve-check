 ...
 *    unknown backdrop. This lab paints its own surfaces from FLAT tokens on
 *    purpose, precisely so its verdict tints are resolvable: every `.verdict`
 *    tone, the tinted lookup-table cells and the hero aside are plain hex.
 *    The shared top bar is not: it builds both its ink and its border from
 *    `color-mix(in srgb, var(--accent) …%, …)`, and every label on it is
 *    therefore invisible to a violations-only gate. The flat-token choice is
 *    a decision this walk keeps honest rather than a fact it may assume —
 *    one `color-mix()` added to `--ok-fill` moves the whole verdict system
 *    into axe's blind spot with nothing turning red.
 *
 ...
 *    `opacity` fade at all: muted text lowers its own lightness (the template
 *    4.2 rule), and `button.act:disabled` deliberately declares `opacity: 1`
 *    and swaps to real tokens rather than fading. So this class is empty here
 *    today, and the walk is what makes that a measurement. Note the one trap
 *    it guards: an `opacity` fade would be exempt on a DISABLED control and
 ...
 *    LOAD-BEARING here, not hypothetical: pane 2 puts four 16x16 lookup
 *    tables inside `.scroll-x { overflow-x: auto }` wrappers, and below about
 *    20rem of column width they really do scroll, so hundreds of hex cells
 *    sit outside their clipper's box at 380px. Content scrolled past a
 *    clipper is not dimmed or partly drawn; it is absent from the frame, and
 *    asking what colour it sits on has no answer. Without this, the walk
 *    would find nothing behind those cells, fall through to WHITE, and invent
 *    a wall of failures at phone width.
 *
 ...
 * glyphs (✓ ✕ ·), the per-cell markers that carry diff and coset state beside
 * their own words (≠ · ▪ ▫), the legend swatches and the `changed` tag on a
 * stepped byte — each painted in a semantic ink on its own tinted fill, which
 * is to say the states this lab exists to show — so `scan()` calls this a
 * second time as
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
     * A driven pass here walks a three-pane document, and the expensive part
     * is pane 2: FOUR 16x16 lookup tables, 1024 `<td>` elements plus their
     * marker spans and row/column headers, all siblings re-walking the same
     ...
     * here that is the four `.scroll-x { overflow-x: auto }` wrappers around
     * pane 2's lookup tables, which really do clip at phone width. The
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: the ten-state trace in pane 1, the defining equations
      // and the AES-comparison note in pane 2, and the three-numbers note in
      // pane 3 are all <details> that ship shut. The
     ...
     *
     * Every `aria-hidden` element on this page is a marker rendered by the UI
     * helpers: the `.verdict-mark` glyph on each verdict, the `.mk` marker
     * inside a matched, mismatched, selected or landed table cell, the `.chg`
     * tag on a byte that changed under a step, the `.swatch` legend squares
     * and the `.pane-num` ordinals — plus the shared header's two SVG marks,
     * which carry no text. Each duplicates the words directly beside it, but
     * each is painted in a SEMANTIC ink on a tinted fill, which is why
     ...
     *
     * Nothing on this page hides a VALUE. No byte, hex cell, coset member or
