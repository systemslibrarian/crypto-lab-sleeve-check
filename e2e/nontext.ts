 ...
 *
 * IN THIS LAB the control-boundary half is the live one. `src/styles.css`
 * draws every control edge from one token, `--border-strong`, and the first
 * draft of that token measured 2.72:1 against `--surface` — this oracle is
 * what named it, on `button.act.ghost`, before the lab ever shipped. Tokens
 * are discarded by overrides, so what is measured is each control AS PAINTED:
 * `button.act` fills with `--accent` and draws its border the same colour, so
 * it has no edge of its own and lives or dies by fill-vs-surround;
 * `button.act:disabled` deliberately keeps `opacity: 1` and swaps to real
 * tokens, so it stays measurable rather than exempt-by-fade; `select` has
 * `appearance: none` and a gradient-drawn chevron; and the `aria-invalid`
 * state recolours an input's boundary to `--bad-border`. That last one only
 * the drive reaches.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks three panes, and the expensive part is pane 2:
     * four 16x16 lookup tables, 1024 `<td>` elements plus their marker spans
     * and headers, all siblings re-walking the same ancestors up to `<body>`.
     ...
     * measuring the theme. This lab ships one theme, so that particular tell is
     * unavailable here, which is a reason to keep the sampler rather than trust
     * a number. This page paints a flat `var(--bg)` LONGHAND on both `html` and
     * `body` — the longhand rather than the `background` shorthand because
     * axe/WebKit miss the shorthand (template 4.2) — so the root resolves on
     * the first try and the `<body>` fallback is dormant. Both
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // does not currently use that idiom — every `.pane-btn` carries a real
      // `--border-strong` edge in both its selected and unselected states —
      // but the per-side test stays, because it is also what correctly
      // measures a control bordered on ONE side only, which the version of
      // this oracle shipped across most of this fleet gets wrong.
